...
* Configuration Layers                                      :TOC_4_gh:noexport:
 ...
 - [[#the-emacs-loading-process-1][The Emacs loading process]]
   - [[#emacs-lisp-files-1][Emacs Lisp files]]
     - [[#loading-a-file-1][Loading a file]]
   - [[#features-1][Features]]
     - [[#the-load-path-1][The load path]]
   - [[#auto-loading-1][Auto-loading]]
   - [[#eval-after-load-1][Eval after load]]
...
* layer 简介
这篇文档是给第一次配置自己的自定义 layer 的朋友准备的一份教程，无论是自己使用还
是贡献给社区，这篇教程有助于帮你理清楚 layer 是如何工作的，和 Spacemacs（或
Emacs）是如何加载包的。
...
* 命名规范
Layers 和 packages. 是怎么回事?

- Package :: 一些 elisp 文件结合在一起形成了一个包，提供一定的功能。包存在于像
     ELPA, MELPA 一类的包管理资源中，或是由（如 github） 第三方服务提供，亦或是
     本地磁盘上。
- Layer :: 一个可以在Spacemacs中启用的配置集合单元。一个 layer 通常集成了一个或
     多个包，以及一些配置代码来紧密结合在一起使他们能够彼此之间配合良好并在
     Spacemace中起到很好的作用。

在写一个层之前，他有助于考虑想实现什么。你想在Spacemacs中集成这些包么？如果是的，
你应该写一个层。你想要实现一个新功能为emacs社区做贡献么？这种情况下，考虑是否要
先写一个报，然后是一个层来使用你的包，
