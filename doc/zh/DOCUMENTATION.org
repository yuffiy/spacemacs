#+TITLE: Spacemacs documentation (Spacemacs 文档)

* Spacemacs documentation (Spacemacs 文档)                                   :TOC_4_gh:noexport:
 - [[Core Pillars (核心理念)][Core Pillars (核心理念)]]
   - [[Mnemonic (低廉的心智负担)][Mnemonic (低廉的心智负担)]]
   - [[Discoverable (_)][Discoverable (_)]]
   - [[Consistent (高度的一致性)][Consistent (高度的一致性)]]
   - [[Crowd-Configured (你不是一个人在配置)][Crowd-Configured (你不是一个人在配置)]]
 - [[Highlighted feature (突出特性)][Highlighted feature (突出特性)]]
...
实时显示可用快捷键的创举. 以及方便的查询系统可以让你快捷地找到可用的 layers 和插件之类的.
...
感谢定义明晰的规范, 相似的功能一般都有相同的快捷键. 而且 Spaemacs 发布的 layer 都强制要求配备文档.

** Crowd-Configured (你不是一个人在配置)
Community-driven configuration provides curated packages tuned by power users
and bugs are fixed quickly.

社区驱动的配置由大神来进行优化和调整, 并且 bug 修复的很迅速.

* Highlighted feature (突出特性)
...
- 重度使用空格来代替功能键(比如ctrl), 让你 *远离肌肉劳损*.
  如果你大拇指有恙, 你还是可以用功能键来使用 Spacemacs 的.

- Contribute easily your improvements and new configuration layers.

- 贡献你自己的改进和新的 layer 很容易.

- *Very active and helpful community* on =Gitter= and =IRC=
  (via Gitter IRC bridge)

- 我们在 =Gitter= 和 =IRC= 上有 *十分活跃且有爱的社区* (通过 Gitter 和 IRC bridge 能找到我们)
